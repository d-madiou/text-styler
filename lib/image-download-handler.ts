import { ImageUrlError, MAX_IMAGE_BYTES, SUPPORTED_IMAGE_TYPES, validateRemoteUrl } from "./image-url";
import { ImageTransport, ImageTransportResponse, validateAndTransport } from "./image-transport";
import type { Lookup } from "./image-url";

const typeExtensions: Record<string, string> = { "image/jpeg": "jpg", "image/png": "png", "image/gif": "gif", "image/webp": "webp", "image/avif": "avif" };
...
export async function handleImageDownload(request: Request, transport: ImageTransport = validateAndTransport, lookup?: Lookup) {
  ...
  try {
    ...
    if ((Number.isFinite(length) && length > MAX_IMAGE_BYTES) || response.body.byteLength > MAX_IMAGE_BYTES) throw new ImageUrlError("TOO_LARGE", "The image is too large.");
